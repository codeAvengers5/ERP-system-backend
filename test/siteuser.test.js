...
describe("ConfirmEmail", () => {
  ...
  describe("LoginSiteUser", () => {
	...
	it("should login a user and return a token", async () => {
	  ...
	  expect(res.json).toHaveBeenCalledWith({
		userInfo: { accountId: "userId", email: "test@example.com", username: "testuser" },
		message: "LoggedIn",
	  });
	});
	...
	it("should return 400 if email or password is missing", async () => {
	  ...
	  expect(res.json).toHaveBeenCalledWith({ Error: "Email and Password can not be Empty" });
	});
  ...
  describe("ForgotPassword", () => {
	it("should send a reset password link", async () => {
	  ...
	  expect(res.status).toHaveBeenCalledWith(200);
	  expect(res.json).toHaveBeenCalledWith({   "msg": "verify with the link", token: "token", id: "userId" });
	});
